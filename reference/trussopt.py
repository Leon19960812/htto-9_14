...
def calcB(Nd, Cn, dof):
    ...
    # Cn[起始节点编号，终止节点编号，杆件长度，布尔值] (connection)
    l, X, Y = Cn[:,2], Nd[n2,0]-Nd[n1,0], Nd[n2,1]-Nd[n1,1]
    # [n2,0] 取出所有终止节点的x坐标
    # [n1,0] 取出所有起始节点的x坐标
    d0, d1, d2, d3 = dof[n1*2], dof[n1*2+1], dof[n2*2], dof[n2*2+1]
    # dof[n1*2] 取出所有起始节点的x方向自由度
    # dof[n1*2+1] 取出所有起始节点的y方向自由度
    # dof[n2*2] 取出所有终止节点的x方向自由度
    # dof[n2*2+1] 取出所有终止节点的y方向自由度
    s = np.concatenate((-X/l * d0, -Y/l * d1, X/l * d2, Y/l * d3)) # sparse matrix values
    r = np.concatenate((n1*2, n1*2+1, n2*2, n2*2+1)) # sparse matrix row indices
    c = np.concatenate((np.arange(m), np.arange(m), np.arange(m), np.arange(m))) # sparse matrix col indices
    return sparse.coo_matrix((s, (r, c)), shape = (len(Nd)*2, m)) 
    # sparse.coo_matrix((data, (row, col)), shape=(nrows, ncols))
    # B的每一行代表一个节点自由度的力平衡方程
#Solve linear programming problem
def solveLP(Nd, Cn, f, dof, st, sc, jc): # st(stress tension):拉伸应力，sc(stress compression):压缩应力，jc(joint cost):节点成本
    l = [col[2] + jc for col in Cn]
    B = calcB(Nd, Cn, dof)
    a = cvx.Variable(len(Cn))  # 定义优化变量截面积a（设计变量）
    obj = cvx.Minimize(np.transpose(l) * a) # 目标函数：最小化体积
    q, eqn, cons= [],  [], [a>=0] # 平衡方程列表，约束方程列表
    for k, fk in enumerate(f): # enumerate遍历时同时获得索引和值 k荷载工况编号 fk荷载工况
        q.append(cvx.Variable(len(Cn))) # （状态变量）每个工况创建一次
        eqn.append(B * q[k] == fk * dof) 
        cons.extend([eqn[k], q[k] >= -sc * a, q[k] <= st * a])
    prob = cvx.Problem(obj, cons)
    # vol = prob.solve() # replaced by Mosek solver for better efficiency
    vol = prob.solve(solver = cvx.MOSEK, \
                     mosek_params = {"MSK_IPAR_INTPNT_BASIS":0})  # 0 is disable and 1 is enable
    q = [np.array(qi.value).flatten() for qi in q]
    a = np.array(a.value).flatten()
    u = [-np.array(eqnk.dual_value).flatten() for eqnk in eqn] # 节点位移向量，虚功原理
    return vol, a, q, u
#Check dual violation
def stopViolation(Nd, PML, dof, st, sc, u, jc):
    lst = np.where(PML[:,3]==False)[0] #未激活杆件索引 (np.where(condition) 返回满足条件的索引)
    Cn = PML[lst] # 未激活杆件列表
    l = Cn[:,2] + jc
    B = calcB(Nd, Cn, dof).tocsc() # CSC = Compressed Sparse Column（压缩稀疏列）格式
    y = np.zeros(len(Cn))
    for uk in u:
...
def trussopt(width, height, st, sc, jc):
    ...
    Nd = np.array([[pt.x, pt.y] for pt in pts if poly.intersects(pt)]) #Nd: (Node)
